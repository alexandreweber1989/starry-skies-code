      ...
      <DialogContent className="sm:max-w-[500px] border-none bg-background/95 backdrop-blur-xl">
        ...
        <div className="space-y-6 py-4">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {quickTemplates.map((t) => (<Button key={t.label} variant="outline" size="sm" className="text-[10px] h-7 rounded-full uppercase tracking-tighter" onClick={() => {
                setTitle(t.title);
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Culto de Domingo Iniciado" className="bg-card/50"/>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Mensagem</label>
              <Textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Descreva o comunicado detalhadamente..." className="min-h-[100px] bg-card/50"/>
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => broadcast.mutate()} disabled={broadcast.isPending} className="gap-2">
            {broadcast.isPending ? (<Clock className="h-4 w-4 animate-spin"/>) : (<Send className="h-4 w-4"/>)}
